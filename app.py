...
with st.sidebar.expander("📖 Strategic Narrative & Guide", expanded=False):
    ...
    **Two-Tier Risk Testing:**
    1. **Survival Risk (Year 1):** Use the 'Year 1 Ramp-Up Delay' preset to test liquidity during the startup phase.
    2. **Valuation Risk (Year 3):** Use the 'Conservative Pricing' preset to test the exit run-rate and EBITDA multiples.
    """)
...
y1_vol_shock = 0 # Specific shock for Year 1 survival testing

if scenario == "Year 1 Ramp-Up Delay":
    st.sidebar.warning("Simulation: -40% Y1 Volume / +10% Y1 Startup Costs")
    y1_vol_shock = -0.40
    cst = 10
...
v_m = st.sidebar.slider("Global Volume Variance", -50, 50, vol, key="vol") / 100
...
results = []
for i in range(3):
    # Apply Year 1 specific shock if the scenario is selected
...
with m2: st.metric("Y1 Survival Margin", f"${df.iloc[0]['Margin']/1e6:.2f}M", help="Operating margin during the startup year.")
with m3: st.metric("3-Yr Liquidity Surplus", f"${df['Cash'].sum()/1e6:.2f}M", help="Cumulative surplus including ERA Grants.")
with m4: st.metric("Lowest Annual Cash", f"${df['Cash'].min()/1e6:.2f}M", help="The minimum total cash inflow in any single year.")
...
with col1:
    ...
    fig.update_layout(
        title="Institutional Liquidity Ladder: Survival (Y1) to Scale (Y3)",
...
with col2:
    ...
    fig_unit.update_layout(
        title="Unit Economics: Performance per Home",
        template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        yaxis_title="USD per HEQ",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
    )
