...
for i in range(3):
    ...
    results.append({
        ...
        "Total Revenue": total_rev, "Margin": margin,
        "ERA": era_grants[i], "Total Cash": margin + era_grants[i],
        "Base": base_rev_targets[i]
    })
...
if view_mode == "Revenue & Growth":
    ...
    with c4: st.metric("3-Yr Growth", "162%", "Base Case")
    fig = go.Figure()
    ...
    fig.update_layout(barmode='stack', title="Revenue Source Breakdown", template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)')
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("3-Yr Total Cash", f"${df['Total Cash'].sum()/1e6:.2f}M")
    with c2: st.metric("Year 2 ERA Grant", "$610K", "Confirmed")
    with c3: st.metric("Year 3 Op Margin", f"${df.iloc[2]['Margin']/1e6:.2f}M")
    with c4: st.metric("Net Margin %", f"{(df.iloc[2]['Margin']/df.iloc[2]['Total Revenue']*100):.1f}%")
    fig_cash = go.Figure()
    ...
    fig_cash.update_layout(barmode='stack', title="Total Cash Inflow (Available for Debt)", template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)')
    st.plotly_chart(fig_cash, use_container_width=True)
    y2 = df.iloc[1]
    fig_w = go.Figure(go.Waterfall(orientation="v", x=["Op Margin", "ERA Grant", "Total Cash"], y=[y2['Margin'], y2['ERA'], y2['Total Cash']], measure=["relative", "relative", "total"], totals={"marker":{"color":BR_GOLD}}, increasing={"marker":{"color":BR_GOLD}}))
    fig_w.update_layout(title="Year 2 Liquidity Walk", template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)')
